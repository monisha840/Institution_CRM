...
import { KPI, EmptyState, Funnel, PageHeader } from "../ui";
...
export default function ScreenReports({ E, session }) {
  ...
  return (
    <div className="page">
      <PageHeader
        sub={"Profit and loss, fee collection, donations and student strength — all filtered by the same date range so the numbers always agree with each other."}
        actions={<><button className="btn accent" onClick={downloadPdf} title="Open a printable, branded PDF report">
      ...
      <div className="card" style={{ padding: "12px 14px", marginBottom: 10, display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center", marginLeft: "auto", flexWrap: "wrap" }}>
          {/* Each label travels with its own field. Flat in one wrapping
              row, "To" stayed at the end of the first line while its input
              wrapped to the second, so the label sat above the wrong
              field. */}
...
// Open a print-styled window with the standard Sirah_CRM header.
//
...
export function openPrintWindow(title, dateLabel, bodyHtml, opts = {}) {
  const school = {
    // No invented fallback: a report with a blank header is honest, one
    // headed with another institution's name is not.
    name:      opts.school?.name      || "",
    trustName: opts.school?.trustName || "",
    regNo:     opts.school?.regNo     || null,
    pan80g:    opts.school?.pan80g    || null,
    contact:   opts.school?.contact   || null,
    brand:     opts.school?.brand     || "Sirah CRM",
  };
