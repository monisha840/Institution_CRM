...
A complete CRM for schools and colleges. Nine roles, one login per
institution, a dashboard per role, and every sensitive action audited.
Backend is Next.js 14 (App Router) with Supabase plus a local JSON file
fallback so the app keeps working even if the cloud is unreachable.

Multi-tenant: one deployment serves a school and a college with entirely
separate students, staff, classes, fees and settings. See the README for
how the tenant is resolved and enforced.
...
# Powered by Sirah CRM
```

Identity is read by `resolveSchool()` from `app_settings.school` (the institution: name, address, contact, affiliation) with `app_settings.trust` supplying the governing body that has to appear on a fee or 80G receipt. Unset fields come back null and the renderer omits the line — an export headed with a blank is honest, one headed with another institution's details is not.
...
### Branded login
- Two-pane login screen: the institution's branding and feature highlights on the left, the credential form on the right.
- A School / College switch is the first control on the page. It re-brands the whole screen — name, strapline, accent colour, feature list, e-mail domain — and decides which institution the credentials are checked against.
- Below 900px the brand panel drops away and the switch moves above the form, so the choice stays the first thing you see on a phone.
- No credentials are shown or pre-filled. Accounts live in the database; the sign-in sheet is generated into `docs/credentials.md` by the seeder.
...
*Generated 2026-04-30 · Sirah_CRM build · Designed & developed by [Sirah Digital](https://sirahdigital.in/).*
